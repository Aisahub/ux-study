...
**Notes** (added 2026-08-12). A Learner may write notes against any Competency, on a second panel of that Competency's page — the taught material and the Learner's own notes being two addresses behind one heading (`DESIGN.md`, The Two Panel Rule) — and read all of them together on one page of their own. They are private without qualification: no colleague and no Maintainer can reach them, and there is no surface that reads them with an address other than the reader's own. They are counted per Competency and never per person, they gate nothing, and they are not an assessment — which is what makes them permissible under the no-cumulative-score rule below rather than an exception to it. The notebook is reached from My page and from each Competency, deliberately not from the navigation rail: its bottom-bar form is full at six marks, which is what a Maintainer already carries.
...
- The specimen is now reachable as well as written (2026-08-14): it is read at `/<lang>/specimen`, linked from the heuristic-evaluation Competency, and withheld until the reader has submitted their own Stage 1 report — before that it would be an answer key for the page they are about to audit, which is the rule the Findings library already keeps. The line that stood here, that no surface served it, was true for the few hours between the two. **Nothing on this list is outstanding.** Everything ADR-0011 decided is built; what is unbuilt from here is whatever gets decided next.
