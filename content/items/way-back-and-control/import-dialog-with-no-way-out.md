...
artefact:
  ...
  ko: >-
    고객 정보 도구의 "연락처 가져오기" 대화상자입니다. 제목 아래에 "영업 —
    북부에 추가할 CSV 파일을 고르세요."라는 한 줄이 있습니다. 그 아래 파일
    줄에는 "선택된 파일 없음"이라고 적힌 회색 상자와 흐린 "파일 선택…" 버튼이
    나란히 있습니다. 그 밑에는 밑줄 친 파란 링크로 "어떤 열이 필요한가요?"가
    있습니다. 맨 아래에는 오른쪽에 컨트롤이 하나뿐입니다. 회색 바탕에 회색
    글자인 "계속"입니다. 제목 줄에는 닫기 표시가 없고, 대화상자 어디에도
    취소가 없습니다.
screen:
  en: |-
    <div class="screen">
