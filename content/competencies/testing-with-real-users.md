...
objective:
  en: >-
    ...
    interface. What is read is the task as it will be said out loud, who is
    being asked to carry it out, and how many of them. What is found is a task
    that names the control to press, a question that tells the participant
    what to think, a group who already know how the thing works.
  ko: >-
    이곳에서 일하지 않는 사람 앞에 인터페이스를 내놓기 위한 계획을 읽고, 그
    계획이 인터페이스가 아니라 계획 자신에 대한 답을 내놓게 될 자리를 찾아낼
    수 있다. 읽는 것은 실제로 소리 내어 전달될 과제 문장, 그 과제를 부탁할
    사람, 그리고 그 인원이다. 찾는 자리는 누를 컨트롤을 이름으로 알려 주는
    과제, 참가자에게 무엇을 생각해야 하는지 일러 주는 물음, 이미 작동 방식을
    아는 사람들로 채워진 명단이다.
roleHint:
  developer:
    en: >-
...
preReadingQuestions:
  - en: >-
      ...
      they, and what does the article require of each for the result to mean
      anything?
    ko: >-
      사용성 조사를 이루는 세 가지 요소가 제시된다. 무엇과 무엇과 무엇이며,
      결과가 뜻을 가지려면 각각이 어떠해야 한다고 말하는가?
  - en: >-
      The article warns that the person running the session can change the
      result without meaning to. Where does it say that influence gets in?
    ko: >-
      이 글은 세션을 진행하는 사람이 의도치 않게 결과를 바꿔 놓을 수 있다고
      경고한다. 그 영향이 끼어드는 자리는 어디라고 말하는가?
  - en: >-
      A number of participants is recommended, but only for one kind of study.
      What is the number, and which kind of study is it for?
    ko: >-
      권장 참가자 수가 제시되지만, 그것은 한 가지 종류의 조사에만 해당한다.
      그 수는 몇이며, 어떤 종류의 조사를 위한 수인가?
source:
