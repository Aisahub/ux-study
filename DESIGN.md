...
**The Reader's Size Rule.** The scale is declared in `rem`, against the reader's own browser default. At the default setting 1rem is 16px and every step renders at the pixel size listed above, so nothing moves for anyone who has not asked it to. A Learner who has raised their browser's default font size gets a page that answers. Accessibility is a hard line on this platform (`PRODUCT.md`), not one of the twelve Competencies; a scale nailed to px ignores the one accessibility preference a reader sets before they ever arrive. Fixed `px` in a type size is now a defect, not a shorthand.
...
**The submitted report keeps the two surfaces** (decided 2026-09-02). Submitting used to end the arrangement: the reveal was one centred column listing what was planted and then what the Learner wrote, both as bare element identifiers, with the page they are about nowhere on screen. A Learner reading `shipping-help-text` had no way to tell which thing on the page that was. It now takes the audit's own shape — the subject on the left, the report on the right — and the report is **two panels behind one heading**, `/audit/<stage>` for what was planted and `/audit/<stage>/findings` for the Learner's own, under the Two Panel Rule and for its reasons. The frame holding the subject is a layout rather than a component each panel draws, so crossing between the panels never reloads the page being talked about; a subject that blanked and scrolled back to the top on every crossing would take away the reason for setting the two side by side at all.

Both panels are grouped by **Competency**, and both by the same one, because they are read against each other: a Learner's accumulated Competencies are their audit checklist (`PRODUCT.md`), and "every hierarchy one was found, the consistency one was missed" is a reading only a shared cut makes possible. A Finding's Competency is the one taught by the Principle the Learner selected while writing it. Nothing counts the groups — a found-count is a per-person total, which this system's own Don'ts refuse; status stays on each card, in the three channels every status here uses.

Every element is named twice on those cards: the words it shows on the page, quoted from the served document, above the identifier the record is keyed by. The identifier alone was the defect; the identifier is also what a Learner pastes into a pull request, so it stays. Beside them sits one control that marks that element on the subject — the same outline the audit's own selection draws, because a Learner has already learned what that outline means and a second treatment for one statement is the Consistency defect the third Competency teaches.

Below `1100px` the subject becomes a collapsible panel above the report rather than a column beside it, and it is open on arrival. The cost is recorded rather than glossed: a panel that started closed would put the report on the first screen, which is what a Learner came back for, but `open` is an attribute and not a style — nothing in CSS sets it per breakpoint, and driving it from a media query in the client is a flash of the wrong state on every load. The brief above the audit answered that wall with two elements, which is free for two paragraphs and would be a second copy of the subject here. So the frame is a strip instead: tall enough to hold an element in its surroundings, short enough that the panel switch and the first card are within a thumb's reach, and one tap puts it away.
...
- **Panel switch:** The same silhouette as the language switcher, and deliberately not a new one — that control is already how this platform says "the same page, in the other version", which is exactly what a Competency's two panels are. Two segments in a white pill, the current one oxblood with white text, the other plain ink, each a link carrying `aria-current="page"` and a `44px` minimum height. Never `role="tab"`: that role promises a scripted widget where arrow keys move between panels without leaving the page, and these are navigation. A trailing count may ride a segment as a quiet detail (72% on the filled segment, ink-72% on the plain one), in the treatment the primary button's "5문항" already uses. Two surfaces carry it now — a Competency's material and notes, and a submitted Self-Audit Report's two panels — and it is deliberately one control rather than two: both say "the same subject, in the other version".
