...
> **Amended 2026-08-05.** Rule 2 binds a *failed* attempt, and until now the passed one was silent for the same reason without the same cause. A pass at 4 of 5 told the Learner only that one of the five had gone wrong, not which — so the one thing they demonstrably did not understand was the one thing the screen would not name. **A passed attempt now explains every drawn item beside the artefact that item was about: the drawn screen, then the keyed answer, the grounds for it, and — on the items that went the other way — what the Learner chose instead.** The artefact is not decoration on that card, it is the subject: the prompts point at it ("this blurred view") and the keyed answers name parts of it ("Option B"), so a card without it records the attempt without explaining any of it — and an Attempt is a permanent record the doorstep links back to, which makes "the Learner still remembers the screen" untrue within a week. "One channel, not two" continues to hold there: the prose artefact stays the frame's accessible name rather than being printed beside the screen, because a paragraph describing the defect does the Learner's rereading for them. The loophole rule 2 closes does not exist here: the answer is worth withholding while it can convert a retry into a pass, and once the gate is behind the Learner there is no retry for it to convert. Nothing is authored for this — the grounds are the `reason` every option already carries, which the Learner read while answering.

Every item rather than only the missed ones, which is where this landed after a first pass showed only the misses. A tick against an item says nothing about *why* it was right, and on four options a Learner who eliminated their way to the answer is indistinguishable from one who saw it — those are precisely the passes worth explaining, and they are invisible to the score. The cost is real and accepted: a draw of 5 puts most of an 8-item pool's answers on one page. It is a cost rather than a leak, for the same reason as above — the gate is already behind this Learner, and there is no attempt left for the answers to convert.
...
- **Amended 2026-07-23: the artefact is drawn, and it is markup rather than an image file.** Stage 1 was authored with every artefact described in prose, which this decision permits — "a described page" is one of the listed formats. Reading it back on screen, it does not hold: a Competency about what a screen's weight and colour are saying, assessed by reading a paragraph about weight and colour, tests comprehension of the paragraph. So an item may now carry a `screen` — an HTML fragment styled by one shared stylesheet — and where it does, that is what the Learner judges.

  Not an image file, which is what "a real screenshot" above implied. A screenshot has to be re-cut for both language variants, cannot be read in a diff when someone changes it, and blurs on the display where the difference between two greys is the whole question. Markup costs less to author than a screenshot, and the discount noted above — sharing an artefact between the variants — is replaced by a better one: the two variants share the stylesheet and differ only in their labels.

  **One channel, not two.** An item with a drawn screen stops printing its prose beside it; the prose becomes the frame's accessible name. A paragraph saying a button is light grey on white answers the question before the screen is looked at, and the Learner is back to reading a description of a defect instead of seeing one. Prose therefore remains the artefact for a Learner who cannot see the screen, and stops being the artefact for everyone else.
