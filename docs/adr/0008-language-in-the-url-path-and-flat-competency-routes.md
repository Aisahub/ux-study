...
>
> **Amended again 2026-07-23: the switch is allowed, and `language` now records only where the attempt began.** The amendment above traded a Learner's ability to read an item in their stronger language for a field whose meaning was tidy. That trade was wrong, and it was wrong in the way this platform teaches: it was made from the record's point of view rather than the reader's. A Learner who meets an English item they cannot parse is being measured on their English, not on their eye for hierarchy — and the assessment is the whole point.
>
> Three things make the switch cheap to allow. An item's keyed option sits at the same index in both languages, and `loadItems` now fails the build if it ever does not. Display order is shuffled from a seed that ignores language, so the four options stay in the same places when a Learner crosses over. And answers are persisted as they are picked, so the navigation costs no work. Nothing in the product groups or compares attempts by their language today; if something ever needs to, it will need a truthful field, and one attempt in a thousand having crossed over mid-way is a smaller lie than the one this platform would be telling by making people sit an exam in a language they did not choose.
