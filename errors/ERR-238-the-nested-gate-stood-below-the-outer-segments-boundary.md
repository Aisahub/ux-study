# [ERR-238] The nested gate stood below the outer segment's boundary

## Summary

[ERR-236]'s resolution did not close the finding, and this is the other
...
| request, Stage 1 reader | ERR-236's fix | this fix |
| --- | --- | --- |
...
ERR-236 added to stand "above the boundary". Above *its own* segment's
...
Neon, and the status was 200 before the answer came back. ERR-236's
...
gated route ERR-236 named — run against *each* distinct gate, not just the
